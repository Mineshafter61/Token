...
class TokenPost extends Listener {
    ...
    val tokenSectionMap: mutable.Map[String, TokenSection] = mutable.Map.empty
    ...
    @EventHandler def onSignClick(event: PlayerInteractEvent): Unit = {
        ...
        if (
          block == null || event.getAction != Action.RIGHT_CLICK_BLOCK || !block.getState
              .isInstanceOf[Sign]
        ) return;
        ...
        val sectionId: String =
            plain.serialize(sign.getSide(Side.FRONT).line(1)).strip();
        val direction: Value = Direction.withName(
          plain
              .serialize(sign.getSide(Side.FRONT).line(2))
              .strip()
              .toUpperCase()
        );
        val maxTrains: Int = Integer.parseInt(
          plain.serialize(sign.getSide(Side.FRONT).line(3)).strip()
        );

        player.sendMessage(tokenSectionMap.mkString)

        val section = tokenSectionMap.getOrElseUpdate(
            sectionId,
            new TokenSection(maxTrains, sectionId)
        )

        val statusMsg = mm(
          s"<blue>ID: <white>${section.name}</white> Direction: <white>${section.currentDir}</white> Max Trains: <white>${section.maxTrains}</white> Current Trains: <white>${section.currentTrains}</white></blue>"
        );
        player.sendMessage(statusMsg);

        if (
          item != null && item.getItemMeta != null && item.getItemMeta.lore != null && plain
            ...
            if (!section.removeTrainOpposite(direction)) {
                player.sendMessage(
                    mm(
                        "<red>ERROR: Section empty/direction invalid! Token could not be received.</red>"
                    )
                );
                return;
            }
            ...
            val receivedMsg = mm(
                s"<green>Token has been received for the section <white>$sectionId</white></green>"
            );
        ...
        // player has no item
            ...
            val insert = section.insertTrain(direction)
            if (!insert) {
                player.sendMessage(
                  mm(
                    "<red>ERROR: Section full! Token could not be generated.</red>"
                  )
                );
                return;
            }
